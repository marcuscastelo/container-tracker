...
Container Tracker is logistics tracking system where operational decisions depend on **accurate interpretation of historical events**.

system processes shipping data sequence of observations which are transformed into:
...
If these rules are implemented in multiple layers (UI, capabilities, routes, utilities), system risks producing **multiple conflicting interpretations of same shipment history**.
...
Because project is partially developed with LLM assistance, there is high risk that logic is unintentionally replicated in UI or orchestration layers.

Therefore system must explicitly define **where domain truth is owned and derived**.

---

# Decision

Domain truth must be derived **only inside owning bounded context**.
...
following responsibilities belong exclusively to **tracking bounded context**.
...
Other layers may only consume results.
...
following patterns violate this ADR.
...
being used outside tracking domain.
...
If feature requires understanding event semantics, it belongs to **tracking domain**, not UI or capability layer.

When uncertain:

1. Prefer calling domain use case
