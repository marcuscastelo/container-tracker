...
This document captures the investigation results about refresh/sync, dashboard behavior, alerts, acknowledge behavior, redestination, ETA rendering and typography.
...
Explanation:
- The `DashboardRefreshButton` component renders the "Sincronizar" UI (label from i18n keys). It implements a cooldown (`DASHBOARD_REFRESH_COOLDOWN_MS`), visual states (`'idle' | 'loading' | 'error'`), blocking while loading, and a spinner/icon for loading. On click it calls the provided `onRefresh` prop and shows loading/error state.
...
Explanation:
- `ShipmentHeader` renders the refresh icon for the Process (Shipment) view using `RefreshButton`. The button disables itself and shows a spinning icon (`aria-busy`) when `isRefreshing` is true. Clicking triggers `props.onTriggerRefresh()` unless the carrier is `'unknown'` (then it opens a dialog).
...
- `refreshShipmentContainers` enqueues per-container sync requests by POSTing to `/api/refresh` and then polls / subscribes to `/api/refresh/status`. See `enqueueContainerRefresh` and `fetchRefreshSyncStatuses` in the same file.
...
Does refresh update:
- the entire process? No — refresh updates tracking-derived fields only (status, statusCode, eta, containers, alerts). See `refreshTrackingDataOnly` which applies only tracking-derived fields.
- a specific container? The POST `/api/refresh` is sent per container; enqueue is per-container and status tracked per sync request id.
- all processes? No — the Process View refresh targets containers belonging to the currently opened process.
...
- Visual feedback present:
  - Spinner icon on the refresh button (SVG with `animate-spin`) when refreshing.
  - `aria-busy` and `disabled` on the button; CSS classes change (opacity / pointer-events).
  - `refreshHint` and `refreshRetry` textual hints shown in `ShipmentHeader` next to the button.
  - `entry-client.tsx` contains a delegated diagnostic handler that shows `alert()` messages and calls `/api/refresh` for elements with `button.refresh-button` (diagnostic code).
...
Is the dashboard auto-refreshing?
- Not in the UI code. The Dashboard uses `createResource` to load read models and a realtime hook `useProcessSyncRealtime` to show ephemeral sync state, but no interval-based auto-refresh is implemented in `DashboardScreen.tsx`.

Does the "Sincronizar" button refresh:
- all processes? Yes — it calls `syncAllProcessesRequest()` (POST `/api/processes/sync`) to run a global sync.
- visible rows? After sync it performs `refetchProcesses()` which reloads the processes listing affecting visible rows.
- selected processes? Per-row sync exists (`syncProcessRequest(processId)` POST `/api/processes/:id/sync`) for single-process sync; but the dashboard "Sincronizar" triggers global sync.

Is the refresh implemented as:
...
- `ShipmentDataView` layout: `ShipmentHeader` (header) → `OperationalSummaryStrip` → grid where left column contains `AlertsPanel` (sticky top), then `ContainersPanel`, then `TimelinePanel`. So alerts appear below the header and above container list and timeline.
...
- Tracking alerts. The process detail API (`getProcessById`) obtains alerts from `trackingUseCases.getContainerSummary` and returns them. The UI consumes those tracking alerts.
...
Which component renders the acknowledge button?
- `AlertsList` (`src/modules/process/ui/components/AlertsList.tsx`) renders the acknowledge UI: a small square button labeled "X" for active alerts and an "Unacknowledge" text button for archived alerts.
...
- Small square button with text "X" for acknowledge; archived alerts have a text button for unacknowledge.
...
Does acknowledge:
- delete alert? No. It updates the `acked_at` timestamp (not delete).
- hide alert? It moves it from active set to archived (active lists filter `acked_at IS NULL`).
- mark as seen? Semantically yes — it marks the alert acknowledged by setting `acked_at`.

Is there any tooltip or label explaining the action?
- Buttons include `aria-label` attributes using i18n keys (e.g., `keys.shipmentView.alerts.action.acknowledgeAria`) to provide accessible labels; the visual "X" button has no textual label beyond aria-label.
...
Which UI component renders the route in dashboard?
...
Is redestination mapped in the UI mapper?
...
Which component renders ETA in the dashboard table?
- `DashboardProcessTable.tsx` → `DashboardProcessRow` renders the ETA column.
...
Is ETA part of the ViewModel or formatted directly in the component?
- ETA value is part of `ProcessSummaryVM` (`eta` field). The component formats ETA via `displayEta` which calls `formatDateForLocale` — so ETA is provided by the ViewModel and formatted in the component.
...
- How dashboard data flows:
  ...
  - The Dashboard "Sincronizar" button calls `syncAllProcessesRequest()` (POST `/api/processes/sync`) and then triggers `refetchProcesses()` and `refetchGlobalAlerts()` to reload the read models. Per-row sync uses `POST /api/processes/:id/sync` for a single process.
...
- How alerts flow:
  - Alerts are created/derived by the tracking domain and returned by the process detail API (`getProcessById`) via `trackingUseCases.getContainerSummary`.
