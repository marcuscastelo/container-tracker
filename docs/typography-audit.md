...
This document contains inventory of font-size usages across UI, compact semantic scale proposal, implementation foundation (tokens added to `src/app.css`) and migration table with recommendations for incremental rollout.

## 1) Font Size Inventory

Summary: searches across `src/` show many distinct literal font-size uses (arbitrary px classes and Tailwind named sizes). codebase already declared small set of numeric variables in `src/app.css` but components use both those tokens and many ad-hoc classes such `text-[10px]`, `text-[13px]`, `text-micro`, `text-label`, `text-sm`, `text-xs`, `text-[9px]`, `text-[22px]` and more. Below are primary tokens/classes found, approximate frequency (from repo search), representative files and notes.
...
- There is mixture of semantic tokens (`text-micro`, `text-label`, `text-caption`, `text-body`) and literal/ad-hoc classes (`text-[10px]`, `text-[13px]`, `text-[9px]`, `text-[22px]`).
- `text-sm` is heavily used default small body size, but multiple components use 13px or 14px their compact size instead of single `text-sm-ui` token.
- Numeric differences between 11px / 12px / 13px / 14px are often visually irrelevant in dense UI. Many adjacent tokens can be collapsed.
- Micro badges use both `text-[9px]` and `text-[10px]`/`text-micro` inconsistently.

## 2) Proposed hierarchy (semantic, compact)

Goal: collapse redundant levels into operational scale with clear semantics for migration. I propose 6-level core scale plus optional hero size for rare page headings.
...
3. `text-caption` — 0.75rem (12px) — line-height: 1rem
   ...
   - Migrate: `text-xs` when used caption, some `text-[12px]` occurrences.
...
Rationale:
- This removes fine-grained 9/10/11/12/13/14 split in favor of semantically named, operational scale.
- scale prioritizes dense legibility and numeric alignment for metrics (tabular-nums) while keeping small number of tokens for maintainability.

## 3) Implementation performed

- Added semantic tokens to `src/app.css` using Tailwind v4 `@theme` block. Tokens added include `--text-micro`, `--text-label`, `--text-caption`, `--text-sm-ui`, `--text-base`, `--text-md`, `--text-lg` and `--text-hero` — each with associated `--*-line-height` token. See `src/app.css` top-level `@theme` block for exact values.

Files changed (foundation only):
- `src/app.css` — replaced old numeric variables with new semantic tokens (no component edits).
...
|`text-sm` (when used primary body)|`text-base`|align default body to 14px token `text-base` for predictable spacing|
...
|page title `text-6xl` (ad-hoc)|`text-hero` (opt-in)|keep hero opt-in token for rare pages|

Notes about migration strategy
- Do not do big-bang replace. Implement tokens and then migrate high-impact components first (see next section).
- Replace literal `text-[NNpx]` only when confident about visual fit; keep literal in place for any complex spacing/visual reason until designer sign-off.

## 5) Next steps & recommended rollout (incremental)
1. Merge foundation token changes (done).
...
4. Keep short visual review checklist: compare before/after for density, wrapping, vertical rhythm; check tabular-nums for numeric tokens.

## 6) Guardrails enforced
- No inline font-size styles introduced by this change.
- Avoid introducing tokens that differ only by 1px unless there is real visual need.
...
- prepare prioritized list of components to migrate (with estimated PR size),
- or perform first incremental migration (example: replace `text-[10px]`/`text-micro` usages in timeline and shipment header with single canonical token).
