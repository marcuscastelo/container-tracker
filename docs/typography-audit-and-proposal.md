...
This report inventories all font sizes found in the application's `src` directory.
...
| `text-base` | 16px / 1rem | 1 | `ShipmentHeader.tsx` | Used once as a responsive breakpoint from `text-sm`. |
...
*   There is a huge amount of granularity, especially in the smaller font sizes (7, 9, 10, 11, 12, 13px). This is excessive and makes the UI inconsistent.
*   `text-[12px]` and `text-xs` are the same size.
*   `text-sm` (14px) is the most used font size, which makes it a good candidate for the default body text.
*   The jump from `text-lg` (18px) to `text-[22px]` is quite large and only used for metrics.
*   The existing CSS variables in `app.css` are not being used by Tailwind's `@theme` and seem to be a previous attempt at tokenization.

## 2. Proposed Typographic Hierarchy

Here is a proposal for a new, simplified typographic hierarchy. It collapses the current 11+ sizes into 6, providing a more consistent and manageable system.
...
*   **`text-micro` (10px):** Consolidates all the very small text used for non-critical information into a single, readable size. 7px and 9px are too small and can cause accessibility issues.
*   **`text-xs-ui` (12px):** Merges `text-[11px]`, `text-[12px]`, and `text-xs`. This will be the go-to for secondary information and dense UI components.
*   **`text-sm-ui` (13px):** This new default size is based on the frequent usage of `text-[13px]` in interactive elements like filters and dropdowns. It offers a good balance between information density and readability for the main content.
*   **`text-md-ui` (14px):** Replaces the most common size `text-sm`. It can be used for emphasized text or for less dense content areas.
*   **`text-lg-ui` (16px):** A combination of `text-base` and `text-lg` for section titles. It provides clear hierarchy without being overly large.
*   **`text-xl-ui` (22px):** This size is reserved for the most important numbers on the screen, like the main metrics on the dashboard.

## 3. Proposed Implementation in `src/app.css`

This is the recommended implementation of the new typographic hierarchy in `src/app.css` using Tailwind's `@theme` feature.
...
| :--- | :--- | :--- |
| `text-[7px]` | `text-micro` | Consolidate into a single micro size |
| `text-[9px]` | `text-micro` | Consolidate into a single micro size |
| `text-[10px]` | `text-micro` | Consolidate into a single micro size |
| `text-[11px]` | `text-xs-ui` | Consolidate into a single small size |
| `text-[12px]` | `text-xs-ui` | Consolidate into a single small size |
| `text-xs` | `text-xs-ui` | Consolidate into a single small size |
| `text-[13px]` | `text-sm-ui` | New default body size |
| `text-sm` | `text-md-ui` | New emphasized body size |
| `text-base` | `text-lg-ui` | Consolidate into a single heading size |
| `text-lg` | `text-lg-ui` | Consolidate into a single heading size |
| `text-[22px]` | `text-xl-ui` | New large metric size |

## 5. Next Steps: Incremental Migration

It is recommended to perform an incremental migration to the new typographic scale. Here is a suggested order of priority:
