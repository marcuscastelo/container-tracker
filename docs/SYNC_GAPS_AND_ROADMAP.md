...
This document records main sync findings from audit, with impact, risk, mitigation, and correction path that preserves boundaries.
...
Duplicate processing, delayed recovery, and operator uncertainty around whether job is still active or abandoned.

**Mitigation**

Add explicit lease renewal/heartbeat from agent runtime without moving semantic tracking logic out of server.
...
`processTarget()` logs errors and waits for lease expiration; it does not immediately terminalize job in normal scrape failure path (`apps/agent/src/agent.ts:735-748`).

**Impact**

queue can remain `LEASED` even when agent already knows attempt failed.
...
Add explicit "mark failed attempt" path distinct from lease-conflict cases.
...
system supports queue-first async refresh and synchronous HTTP endpoints that wait for queue completion (`src/modules/tracking/interface/http/refresh.controllers.ts:58-69`, `src/modules/process/interface/http/process.controllers.ts:395-495`).
...
`ShipmentView` still calls `/api/refresh` per container instead of newer process refresh endpoint (`src/modules/process/ui/ShipmentView.tsx:333-356`, `src/routes/api/processes/[id]/refresh.ts:1-5`).

**Impact**

Detail refresh bypasses more explicit process-scoped API.
...
Move detail page to process refresh endpoint and use existing `mode` contract.
...
3. Reuse same status/realtime follow-up model.
...
process BC currently carries orchestration responsibilities that architecture docs reserve for capability-style composition.
...
Introduce capability layer for sync/process-detail aggregation while keeping tracking derivation in tracking BC.

**Correction plan**

1. Create dedicated sync/process-read capability.
...
`trackingUseCases.fetchAndProcess()` still exists, but no active route/controller call site was found during audit (`src/modules/tracking/application/tracking.usecases.ts:67-90`, `src/modules/tracking/application/usecases/fetch-and-process.usecase.ts:43-98`).

**Impact**

Docs and future contributors can misread current runtime split.

**Risk**

Accidental reintroduction of direct provider fetches in HTTP server.

**Mitigation**

Make queue-first agent fetch explicit documented default and narrow unused path.

**Correction plan**

1. Remove public facade export if no caller needs it.
2. Or move it behind maintenance-only/admin-only path with explicit naming.
...
But no metrics, tracing, or job lifecycle analytics pipeline was found (`src/modules/tracking/interface/http/agent-enroll.controllers.ts:156-173`, `src/modules/tracking/interface/http/agent-enroll.controllers.bootstrap.ts:238-257`, `apps/agent/src/agent.ts:740-855`).

**Impact**

Harder to answer operational questions such failure rate by provider, queue latency, or stuck lease rate.
...
3. Add metrics/tracing incrementally once job model is stabilized.

### Finding 8: Backfill is latent but not operationalized

**What**

`isBackfill` exists in pipeline and alert derivation, but no runnable backfill path was found in audited sync runtime (`src/modules/tracking/application/orchestration/pipeline.ts:67-75`, `src/modules/tracking/features/alerts/domain/derive/deriveAlerts.ts:127-275`).

**Impact**

domain is partially ready for backfill semantics, but operations are not.
...
Design backfill first-class sync mode before any bulk onboarding/backfill feature ships.

**Correction plan**

1. Add explicit backfill job type/mode.
2. Define live-vs-backfill behavior at queue contract.
3. Reuse existing `isBackfill` semantics in server pipeline.
...
2. Add immediate failure marking for agent scrape/ingest failures instead of waiting for lease expiry (`apps/agent/src/agent.ts:735-748`).
3. Add lease heartbeat/renewal for long-running jobs (`supabase/migrations/20260225_01_agent_sync_mvp.sql:59-98`).
4. Add focused sync ops panel on top of `GET /api/processes/sync-status` and `sync_requests` timestamps (`src/modules/process/interface/http/process.controllers.ts:163-186`, `supabase/migrations/20260306_02_process_sync_observability_and_alert_ack_metadata.sql:28-31`).
...
- Choose async queue-first refresh canonical user-facing pattern.
- Document `/api/processes/:id/refresh` preferred process-detail entry point.
- Keep `/api/processes/sync` only temporary compatibility.

### Phase 2: Harden job runtime

- Add heartbeat/renew.
- Add immediate failure reporting from agent.
...
- Extract cross-BC sync/read-model composition into dedicated capability.
- Keep tracking BC only owner of canonical tracking semantics.

## Questions Still Open

- What maximum attempt count should convert retried job into terminal failure or dead-letter?
- Should backfill share `sync_requests` or use separate queue contract?
