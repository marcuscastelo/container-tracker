...
1) Repositórios retornando unions { success: boolean } (contrato SupabaseResult)
   ...
   - Tipo: CRÍTICO
   - Por que viola: Documento, Seção 7.1 e Checklist (Seção 13) proíbe padrão `repository` que retorna `{ success: boolean }` — repositories devem lançar exceções (`throw`) em falhas e expor assinaturas claras. uso de `SupabaseResult` propaga padrão de HTTP/código para camadas superiores e obriga checagens condicionais por todo application layer.
   - Sugestão: migrar contrato do repository (domain `*Repository` interfaces) para retornar tipo esperado diretamente (ex.: `Promise<Observation[]>`, `Promise<Snapshot | null>`, `Promise<void>`) e lançar `InfrastructureError`/erros específicos em caso de falha. Como correção incremental: introduzir adaptador na borda (infra) que converte atual `SupabaseResult` para `throw` (lançar) e, em seguida, atualizar callers para remover checagens `if (!result.success)`.
...
2) Infra recebe tipos de domínio (NewObservation/NewSnapshot) em vez de Records
   ...
   - Sugestão: definir `InsertObservationRecord` / `InsertSnapshotRecord` (application ↔ infra `Record`) e converter (mapeador infra) `Record → Row`. Como correção incremental: adicionar pequenas funções `toObservationInsertRecord(domainNewObservation)` na camada application antes de chamar infra; ou adicionar adapter em `infrastructure/` que recebe `NewObservation` e produz/valida `InsertRow` internamente, mas com types explícitos `Record` na assinatura pública do repository (migrar interface `ObservationRepository` aceitar `Record`).
...
3) Mappers e validação runtime espalhados/duplicados dentro de infra
   ...
   - Tipo: MÉDIO
   - Por que viola: Seção 6.1 define que mappers de persistência devem viver em `modules/<mod>/infrastructure/persistence/*.mappers.ts`. Aqui mapeamento e validação (`safeParse`) estão inline no repo, que dificulta reuso/testes e misturam responsabilidades.
...
4) Uso de `SupabaseResult` no domínio (interfaces do repository)
   ...
   - Tipo: CRÍTICO
   - Por que viola: interface do repository no domínio usa `SupabaseResult` (tipo infra/transversal). Seção 7 exige que interface do repository seja domain-agnostic (tipos de domínio/leitura) e que erros sejam throw. Usar `SupabaseResult` colapsa infra com domínio.
   - Sugestão: alterar tipos do `ObservationRepository` (e demais repos) para retornar tipos esperados ou `Promise<T | null>` e lançar erros; remover dependência de `SupabaseResult` do domínio. Implementação incremental: adicionar adapter que converte `SupabaseResult` → `throw` e manter interface atual por enquanto; depois migrar callers.
...
1) Domain model usa snake_case e Zod — nomes de campo iguais ao Row/DB
   ...
   - Por que viola: Seção 4 e 11 do documento deixam claro que domínio deve usar tipos canônicos (camelCase e value objects) e que `Row` / snake_case devem ficar na infra (Seção 3 e 6.1). Atualmente `ProcessSchema` usa `created_at`, `updated_at` e campos em snake_case no domínio — isso faz domínio parecer DTO/Row e permite vazamento de `Row` para camadas superiores (application/UI).
   - Sugestão: migrar domínio para usar `createdAt`, `updatedAt`, e value objects (`ProcessEntity` / `ProcessAggregate`) com factories `createProcessEntity`. Como correção incremental: manter `ProcessSchema` para validação de inputs, mas criar typedefs `Process` (camelCase) e mapeador infra `rowToProcess`/`processToDomain` que converta snake_case → camelCase. Evitar expor `ProcessSchema` diretamente como tipo de domínio em `export type Process = z.infer<typeof ProcessSchema>`.
...
2) Zod schemas de transporte/localizados no domínio (`CreateProcessInputSchema` em domain)
   ...
   - Tipo: ALTO
   - Por que viola: Seção 6 e 11: Request/Response DTOs (Zod) pertencem à camada Interface/HTTP (`interface/http`). Aqui schema `CreateProcessInputSchema` vive em `domain/`, misturando responsabilidade. doc recomenda que mapeamento HTTP → Command ocorra em `interface/http`.
   - Sugestão: mover `CreateProcessInputSchema` para `src/modules/process/interface/http/process.schemas.ts` e criar `http.mappers.ts` que converta `Request DTO → Command` (application). Como correção incremental: deixar arquivo mas exportar TODO e criar forwarding module em `interface/http` que importa/encapsula schema antes de mover tudo gradualmente.
...
3) Repositório de processo (infra) aceita `NewProcess` (domain) e retorna `SupabaseResult`
   ...
   - Tipo: CRÍTICO
   - Por que viola: Seção 6.1 e 7. repository deve aceitar `Record` para persistência e não propagar `SupabaseResult` para domínio. Aceitar `NewProcess` (domínio) em `create(process: NewProcess)` mistura camadas.
   - Sugestão: definir `InsertProcessRecord` (application ↔ infra) e alterar assinatura `create(record: InsertProcessRecord): Promise<Process>` (ou lançar erro). Como correção incremental: adicionar pequeno adapter `createFromDomain(newProcess: NewProcess)` internamente no arquivo que converte Domain → Record, e alterar interface pública do repository para aceitar `Record` (refatorar callers em application para produzir `Record`). Também remover `SupabaseResult` e adotar `throw` semantics.
...
4) Uso de Partial<Omit<Process,...>> em application (update) — Partial de Entity
   ...
   - Sugestão: criar `UpdateProcessRecord` (Record) com explicitidade dos campos updatáveis e usar esse tipo na assinatura de `processRepository.update(processId, record: UpdateProcessRecord)`. Em aplicação, mapear `CreateProcessInput` → `UpdateProcessRecord` explicitamente (já há código que monta `updates` — substitua uso de `Partial<Omit<Process,...>>` pelo novo tipo).
...
5) Routes/controllers chamando repository diretamente
   ...
   - Tipo: CRÍTICO
   - Por que viola: Seção 10 (SolidStart: routes, controllers e separação) e Checklist (Seção 13) proíbem controller chamando repository diretamente. `routes/api/processes.ts` tem código que usa `supabaseProcessRepository.fetchContainerByNumber` para ajudar resolver conflito — isso acopla camada HTTP à infra.
   - Sugestão: criar application service / use case `resolveContainerOwner(containerNumber)` ou expor via `processUseCases`/facade método para resolver proprietário do container; migrar chamadas do route para esse use case. Correção incremental: extrair função utilitária em `modules/process/application` que encapsula lógica e use- da rota.
...
6) Aggregate: inexistente ou comentado — regras de invariantes fora do domínio
   ...
   - Tipo: MÉDIO/ALTO
   - Por que viola: Seção 12 define Aggregates como responsáveis por invariantes. Aqui aggregate está ausente (comentado) e regra `CannotRemoveLastContainer` é tratada no application/usecases (ex.: container reconcile). Isso reduz coerência e dispersa regras de negócio.
   - Sugestão: implementar Aggregate (mesmo simples) `ProcessAggregate` com operações `addContainer`, `removeContainer` que garantam invariantes e publicar testes unitários mínimos. Como correção incremental: extrair lógica de checagem de `reconcile` para função pura em `domain/` e usar essa função tanto no usecase quanto em testes.
...
1) Container repository usa `throw` (bom) — contraste com outros repos
   ...
   - Por que: Este repo lança `InfrastructureError` em vez de retornar unions — isso está alinhado com Seção 7.1. No entanto, uso inconsistente (container repo lança, process/tracking usam `SupabaseResult`) cria risco de ambiguidade para callers.
   - Sugestão: preferir padronizar todos repos para `throw` (migrar tracking/process repos) para reduzir inconsistência.
...
1) UI e presenters consumindo tipos com snake_case
   ...
   - Tipo: ALTO
   - Por que viola: Seção 4 e 6.1 dizem que snake_case deve ficar na infra e que cada fronteira deve ter seu tipo. Aqui UI/presenter consomem objetos com `created_at`, `container_number`, etc. Isso é consequência do domínio modelado com snake_case e do fato de que mapeamentos HTTP/VM não estão claramente separados.
   - Sugestão: definir Response DTOs em `modules/<mod>/interface/http/*.schemas.ts` (já há `shared/api-schemas` — consolidar) e criar mappers `http.mappers.ts` para transformar Entity/Result → Response DTO no controller. Em UI, consumir ViewModel (retornado por presenter) que use camelCase e tipos próprios. Correção incremental: adicionar thin mappers no `processPresenter` que convertam snake_case → camelCase e garantir que UI uses ViewModel.
...
2) Controllers (routes) fazendo mapeamentos manuais de entidade para response
   ...
   - Tipo: MÉDIO
   - Por que viola: Seção 10.2 — controller deve mapear `Entity/Result → Response DTO` via `interface/http` mappers. Aqui mapeamento é feito inline no route, espalhando lógica.
...
1) Zod schemas e tipos de transporte sendo usados entre camadas
   ...
   - Sugestão: mover HTTP-facing schemas para `interface/http` (ou consolidar em `shared/api-schemas` mas **usar como DTO**, não como domain types). Domínio deve declarar suas próprias types/factories.
...
2) Spreads e merges entre objetos de diferentes camadas
   ...
   - Sugestão: construir explicit object with exact fields required for return type (e.g., create `ProcessWithContainers` by assembling fields explicitly) — ou use specific mapper function `rowToProcessWithContainers(row)`.
...
2) Mistura de responsabilidades entre `domain/` e `interface/` (Zod schemas em `domain/`)
   - Vários schemas de request/response e validações de transporte estão em `domain/` (ex.: `CreateProcessInputSchema` em `domain/processStuff.ts`). Isso dificulta evolução e quebra regra "cada fronteira muda tipo" (Seção 1 e 6).
...
3) Falta (ou comentário) de Aggregate para `Process`
   ...
   - Correção recomendada: implementar aggregate minimal, extrair lógica de invariant check para domínio.
...
- Principais padrões incorretos recorrentes:
 ...
 2. Tipos snake_case e Zod de transporte/row vazando para domínio e UI (domain com nomes de colunas).
...
- Ordem recomendada de refatoração (incremental):
 ...
 2. Extrair/centralizar infra mappers (`rowToX`) presentes inline para `infrastructure/persistence/*.mappers.ts` e garantir que snake_case exista em `Row` e mappers.
 3. Mover schemas HTTP (ex.: `CreateProcessInputSchema`) de `domain/` → `interface/http/` e criar mapeadores HTTP → Command (application). Atualizar routes para usar useCases/facades em vez de chamar repos.
  ...
  5. Implementar (ou reativar) `ProcessAggregate` com invariantes e mover validações de invariantes para domínio.

---

## Trade-offs e decisões ambíguas

- Uso de Zod no domínio: Zod é útil para validação runtime (especialmente ao mapear rows). recomendação do documento é separar schemas de transporte dos tipos de domínio. abordagem prática é manter Zod para validação de `Row → domain` dentro da infra (mappers), mas não usar Zod como definição principal do modelo de domínio exportado. Isso equilibra segurança runtime com separação de responsabilidades.

- Padronização para `throw` em repositoria exige mudanças em muitos callers. Para evitar regressões, introduzir adapters que façam transição (compat layer) é abordagem mais segura.
