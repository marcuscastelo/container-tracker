...
Open requests are deduplicated by unique partial index on `(tenant_id, provider, ref_type, ref_value)` where status is `PENDING` or `LEASED`. `enqueue_sync_request()` inserts new `PENDING` row or returns existing open row with `is_new=false` (`supabase/migrations/20260225_02_refresh_queue_first.sql:23-117`).
...
No lease heartbeat or renewal endpoint/function was found. only confirmed recovery path for abandoned work is lease expiry followed by re-leasing (`supabase/migrations/20260225_01_agent_sync_mvp.sql:80-98`, `src/modules/tracking/interface/http/agent-sync.controllers.bootstrap.ts:53-118`).
...
Set by `lease_sync_requests()` when agent claims work (`supabase/migrations/20260225_01_agent_sync_mvp.sql:88-96`).
...
queue deduplicates open requests by target/provider while row is `PENDING` or `LEASED` (`supabase/migrations/20260225_02_refresh_queue_first.sql:23-117`).
