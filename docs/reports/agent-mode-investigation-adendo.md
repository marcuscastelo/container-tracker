...
Objetivo
--------
Complementar investigação anterior com decisões arquiteturais necessárias antes de implementar suporte operacional Linux via `systemd`. Este adendo foca em quem é responsável por quê no runtime, sem propor implementação técnica (sem unit files, sem código, sem comandos systemd).

Legenda de evidência
- confirmado: encontrado no código ou documentação canônica
- inferido: dedução razoável partir do código/arquivos existentes
...
1. Quem é supervisor primário em tempo de execução (systemd vs supervisor.js vs híbrido).
2. Quem controla updates (self-update runtime vs package manager vs híbrido) e protocolo de staging/rollback.
...
- supervisor.js (atual)
  ...
  - Operacional: reinício por crash e gate de saúde ficam no supervisor; systemd atuaria como launcher/boot-start.
  - Evidência: código de `supervisor.ts` (confirmado).

- systemd
  - Papel: tornar-se supervisor real do processo runtime.
  - que migrar: restart policy, start at boot, health checks via `Watchdog` / `systemd-notify` (opcional), e responsabilidade de reinicios automáticos.
  ...
  - Operacional: supervisor.js teria que ceder responsabilidades (staging + rollback), ou funcionar como agente de orquestração interna sem lifecycle supervision.
...
- Híbrido (systemd + supervisor.js)
  ...
  - Vantagens: mantém lógica de rollout existente; ganha integração distro-level para boot/start/logs.
...
- Self-update (status quo: runtime controla `runUpdateCheck()`)
  ...
  - Rollback: já existe staging + rollback logic (confirmado); precisa garantir atomicidade e integridade; operação auditável se agent persistir release-state e logs (requer explicit contract de persistência).
...
- Híbrido
  ...
  - Requisitos: definir claramente quem executa substituição de binários, e quem realiza ativação (exit+restart). Ex.: agent baixa e valida artefato → escreve `release-state.json` com metadata e exit(42) → supervisor/systemd reinicia e ativa.
...
- `AGENT_DATA_DIR` — deve resolver completamente diretório de runtime; quando presente, prevalece (confirmado: repo já observa alguma env var? não encontrado — inferir necessidade).
...
- agent não deve depender de `%LOCALAPPDATA%` semantics no Linux (não encontrado) — qualquer uso de `path.win32` deve ser substituído por camada de abstração que consome `AGENT_DATA_DIR` ou resolve por plataforma (divergente doc vs código).
...
- CLI control tool (recomendado como mínima)
  ...
  - Utilidade: cobre maioria dos cenários operacionais sem GUI; permite automação e scripting.
...
- multi-target
  - Ideal longo prazo: oferecer `.deb`/`.rpm` + container images + tarball para dev/manual.
...
- Update restart
  - ownership: runtime inicia fluxo (download/validação/staging), mas request restart via agreed exit code; systemd ou supervisor realiza restart.
...
1. Adotar modelo híbrido com contrato claro: systemd é supervisor primário para boot/restart/logging; `supervisor.js` mantém lógica de rollout/staging/rollback, mas opera como process-internal orchestrator (não como primary lifecycle manager).
...
2. Update strategy: preferir híbrido.
   ...
   - Ativação por restart fica cargo do systemd (ou supervisor se for caso de deploy sem systemd).
...
3. Paths: formalizar `AGENT_DATA_DIR` e defaults Linux:
   ...
   - exigir que todos cálculos de path usem abstração cross-platform (resolver uso `path.win32`) — tarefa técnica ser priorizada.
...
2. Criar especificação de `AGENT_DATA_DIR` e migrar código para usar camada de path-abstraction; eliminar `path.win32` usage.
...
Anexos / Notas
- Marcações de evidência: ver: `tools/agent/supervisor.ts`, `tools/agent/agent.ts`, e referências `runUpdateCheck()` no código base (confirmado). Qualquer conflito entre docs e código foi marcado como "divergente".

### Adendo 2 — Arch Linux / AUR-compatible packaging

Como ambiente operacional real de validação, Linux deve incluir também alvo **Arch/AUR-compatible**, mesmo sem publicação no AUR oficial.

Objetivo:
- permitir instalação local com `makepkg -si`
- validar contrato real de packaging Linux em Arch
...
- integração com supervisor model escolhido
- 
Fim do adendo.
