...
# 1. Sufficiency Check
- docs suficientes: documentos listados (`docs/AGENT_ARCHITECTURE.md`, `docs/SYNC_*`, `docs/SYNC_CODEMAP.md`, `docs/mvp-agent-sync.md`) cobrem arquitetura, contratos e fluxos do agent e servidor; são suficientes para avaliação inicial.
...
- `agent:run` (Linux) = build + `scripts/agent/run-linux.sh` que executa `node .../supervisor.js`; supervisor arranca/monitora runtime filho e implementa health-gate, activation/rollback e restart-for-update.
...
- Windows release: instalador (ISS/Inno) + Task Scheduler + tray (PowerShell) oferecem UX operacional (status, abrir logs, reiniciar). `rebuild-restart` recompila bundle e dispara instalador e tasks.
...
|persisted state|`config.env`, `runtime-health.json`, `release-state.json`|same|same in `%LOCALAPPDATA%`|N/|N/|
|logging|supervisor rotates `logs/supervisor.log`; agent writes logs; `updater.log`|same|tray writes `agent.out.log`/`agent.err.log` and provides UI to open|N/|N/|
|restart|supervisor auto-restart; agent uses exit code 42 -> supervisor restarts for update|supervisor activates staged release and restarts|Task Scheduler + tray restart/monitor|N/|installer triggers tasks to start services|
...
|user-facing surface|none native in Linux repo (logs + health files)|same|tray UI (PowerShell) with status/restart/logs|N/|N/|
|observability|heartbeat to `/api/agent/heartbeat` + `runtime-health.json`|same|same + tray provides easy log access|N/|N/|
|-specific deps|Node >=22, supervisor (JS)|release includes platform adapters|Windows-specific installer + schtasks + PowerShell tray|N/|Windows-only (ISCC + PowerShell)|

# 4. Code/Doc Evidence
- Entrypoint / runtime core
  - `tools/agent/agent.ts` — função `main()` (bootstrap, scheduler, realtime, update-check, shutdown) — CONFIRMADO
  - `docs/AGENT_ARCHITECTURE.md`, `docs/mvp-agent-sync.md` — doc cobrindo fluxo — CONFIRMADO (alguma linguagem antiga sobre updater foi divergente, ver abaixo)
- agent:run (Linux)
...
- que já está pronto (prós):
  ...
  - release layout é gerado e contém artefatos necessários (`app/dist/agent.js`, `supervisor.js`, `updater.js`) — viável para execução por systemd (CONFIRMADO).
- que está faltando (necessário para operador):
...
# 7. Risks and Gaps
- Crítico
  - ausência de tray/UX no Linux se paridade UX for exigida.
...
1. Qual é expectativa mínima de UX local no Linux? (só logs/ctl via CLI, ou equivalente tray?)
2. Devemos fornecer unit file systemd e packaging (deb/rpm/flatpak) no repo ou deixar como artefato operacional externo?
3. Confirmar: em máquinas Linux, `resolveDefaultDataDir()` (que usa `path.win32`) não fará agente escrever em caminhos Windows-style — favor confirmar execução real com `agent:run` em Linux e reportar `runtime-health.json` path.
4. Política esperada para auto-update: automática por canal `stable` (atual comportamento) é aceitável, ou prefere atualizar via instalador/ops?
