...
Critério: registrar **decisão operacional repetida** (não função parecida).
...
|D9|Política de leitura/tail de logs (bounds e canais) em mais de owner|`control-core/public-control-files.ts:60`, `control-core/public-control-files.ts:209`; `control-core/local-control-service.ts:332`, `control-core/local-control-service.ts:357`; `cli/ct-agent.ts:546`; `log-forwarder.ts:402`|UX e semântica de logs podem divergir por interface|alta|`observability/`|
...
duplicações mínimas solicitadas estão cobertas:
