...
Date: 2026-03-13
Source baseline: `docs/reports/code-report/code_report_20260313_033126.txt`
...
|---|---|---|---|---|---|---|
|`src/capabilities/dashboard/application/dashboard.navbar-alerts.readmodel.ts`|Capability dashboard maintainers|refactor-local|Extracted message-contract, grouping/sorting, and shared model helpers; keep read model composition entrypoint|resolved|P0|Next baseline|
...
|`src/capabilities/dashboard/interface/http/tests/dashboard.controllers.*.test.ts`|Capability dashboard maintainers|guideline-checklist|Suite partitioned by behavior family (`operational-summary`, `monthly-chart`, `navbar-summary`) with shared harness helper|resolved|P0|Next baseline|
...
|`src/modules/process/ui/validation/processApi.validation.ts`|Process UI maintainers|refactor-local|Separated UI validation from transport helper exports (API calls now from `ui/api/process.api.ts`)|resolved|P0|Next baseline|
...
- if same ambiguity repeats across multiple modules, mark `adr-future-hold`
