...
async function writeAgentEntrypointShims(distDir: string): Promise<void> {
  ...
  const registerImport = "import './tools/agent/runtime/register-alias-loader.js'\n"
  await fs.writeFile(
    agentShimPath,
    `${shimBanner}${registerImport}await import('./tools/agent/supervisor.js')\n`,
    'utf8',
  )
  await fs.writeFile(
    updaterShimPath,
    `${shimBanner}${registerImport}await import('./tools/agent/updater.js')\n`,
    'utf8',
  )
}
