...
function matchesLayerRule(relativePath, rule) {
  if ((rule.files ?? []).includes(relativePath)) {
    return true
  }
...
function classifyLayer(relativePath, scope) {
  for (const [bucketName, rule] of Object.entries(scope.layers)) {
    if (matchesLayerRule(relativePath, rule)) {
      return bucketName
    }
  }
...
export function renderCoveragePolicyMarkdown(report) {
  ...
  const sections = [
    ...
    '- Layer breakdown follows canonical directories first and explicit shared/presenter overrides second.',
    '- `unclassified` is diagnostic in v0 and does not fail the build.',
    '- `unclassified` means a file is in scope but does not match any explicit module/layer rule yet.',
    '- Tracking-critical semantic coverage is documented in `docs/plans/coverage-tracking-critical-matrix.md`.',
...
function printConsoleSummary(report) {
  ...
  console.log('')
  console.log('- Tracking critical matrix: docs/plans/coverage-tracking-critical-matrix.md')
}
