-- 20260402_02_add_one_provider_support
-- Adds ONE to sync provider checks, agent capabilities, and the paced sync scheduler.
...
check (provider in ('maersk', 'msc', 'cmacgm', 'pil', 'one'));

update public.tracking_agents
set capabilities = case
  when capabilities @> '["one"]'::jsonb then capabilities
  else capabilities || '["one"]'::jsonb
end
