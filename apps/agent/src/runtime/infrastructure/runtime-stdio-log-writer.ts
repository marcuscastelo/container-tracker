...
function getChunkSizeBytes(chunk: Buffer | string): number {
...
export type ChunkWritable = {
  readonly destroyed: boolean
  write: (chunk: Buffer | string) => boolean
  end: (callback?: () => void) => void
  once: (
    eventName: 'close' | 'error' | 'finish' | 'open',
    listener: (...args: readonly unknown[]) => void,
  ) => ChunkWritable
  on: (
    eventName: 'error' | 'open',
    listener: (...args: readonly unknown[]) => void,
  ) => ChunkWritable
  destroy: (error?: Error) => void
}

export type RotatingChunkWriter = {
  write: (chunk: Buffer | string) => void
  close: () => Promise<void>
}

type RotatingChunkWriterDeps = {
...
function resolveDefaultDeps(): RotatingChunkWriterDeps {
  return {
...
export function createRotatingChunkWriter(
  command: {
    readonly logPath: string
    readonly maxSizeBytes: number
  },
  providedDeps?: Partial<RotatingChunkWriterDeps>,
): RotatingChunkWriter {
  const defaultDeps = resolveDefaultDeps()
  const deps: RotatingChunkWriterDeps = {
  ...
  let stream: ChunkWritable | null = null
  let closed = false
  let rotating = false
  let bufferedBytes = 0
  let nextStreamOpenAttemptAtMs = 0
  let lastWarnedErrorMessage: string | null = null
  let buffer: (Buffer | string)[] = []

  const dropOverflowingBufferedChunks = (): void => {
  ...
  const bufferChunk = (chunk: Buffer | string): void => {
    buffer.push(chunk)
  ...
  const handleStreamError = (failedStream: ChunkWritable, error: unknown): void => {
