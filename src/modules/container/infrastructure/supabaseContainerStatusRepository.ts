...
export const supabaseContainerStatusRepository: ContainerStatusRepository = {
  async fetchAll(): Promise<readonly ContainerStatus[]> {
    ...
    console.log('repository/container/fetchAll: fetched rows=', data)
    ...
    return data.map((row) => {
      try {
        ...
        if (status == null) {
          return { container_id: cid, status: {} }
        }
        ...
        return { container_id: String(row?.container_id ?? 'unknown'), status: (row?.status as Record<string, unknown>) ?? {} }
      }
    })
  },
