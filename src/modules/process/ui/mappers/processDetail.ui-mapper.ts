...
export function toShipmentDetailVM(
  ...
  const containers = data.containers.map((container) => {
    ...
    return {
      ...
      carrierAssignmentMode: container.carrier_assignment_mode,
      carrierDetectedAt: container.carrier_detected_at ?? null,
      carrierDetectionSource: container.carrier_detection_source ?? null,
  ...
  return {
    ...
    carrier: data.default_carrier_code ?? data.carrier ?? null,
    defaultCarrierCode: data.default_carrier_code ?? data.carrier ?? null,
    carrierMode: data.carrier_mode,
    lastResolvedCarrierCode: data.last_resolved_carrier_code ?? null,
    carrierResolvedAt: data.carrier_resolved_at ?? null,
    effectiveCarrierSummary: data.effective_carrier_summary,
    effectiveCarrierCodes: data.effective_carrier_codes ?? [],
