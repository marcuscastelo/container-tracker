...
function registerEffectiveCarrierProjectionTests(): void {
  it('derives effective carrier summary from container carriers', () => {
    ...
    expect(requireAt(result, 0).effectiveCarrierSummary).toBe('MIXED')
    expect(requireAt(result, 0).effectiveCarrierLabel).toBe('MSC +1')
    expect(requireAt(result, 1).effectiveCarrierSummary).toBe('UNKNOWN')
    expect(requireAt(result, 1).effectiveCarrierLabel).toBe('Unknown')
  })
}
...
describe('toProcessSummaryVMs', () => {
  ...
  registerEffectiveCarrierProjectionTests()
})
