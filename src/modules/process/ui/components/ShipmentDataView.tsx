...
import { ShipmentMixedCarrierBanner } from '~/modules/process/ui/components/ShipmentMixedCarrierBanner'
...
export function ShipmentDataView(props: ShipmentDataViewProps): JSX.Element {
  ...
  const effectiveCarrierCodes = () => props.data.effectiveCarrierCodes ?? []
  ...
  return (
    <div class="space-y-4">
      ...
      <div class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div class="space-y-4">
          <Show when={props.data.effectiveCarrierSummary === 'MIXED'}>
            <ShipmentMixedCarrierBanner
              effectiveCarrierCodes={effectiveCarrierCodes()}
              onReviewContainers={() => {
                const element = globalThis.document.getElementById('shipment-containers')
                element?.scrollIntoView({ behavior: 'smooth', block: 'start' })
              }}
              onNormalizeAutoContainers={() => props.onNormalizeAutoContainers(props.data.id)}
            />
          </Show>
