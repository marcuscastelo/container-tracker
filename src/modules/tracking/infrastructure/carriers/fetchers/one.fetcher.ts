...
type OneSecondaryFetchResult = {
  readonly payload: unknown | null
  readonly meta: OneEndpointMeta
...
function validateSearchResult(command: {
  ...
  readonly meta: OneEndpointMeta
  readonly parseError: string | null
  readonly searchPayload: OneSearchResponse | null
  readonly searchItem: OneSearchContainer | null
} {
  ...
  if (!isSuccessfulCarrierEnvelope(payload)) {
    errors.push(
      `ONE search returned unsuccessful carrier envelope status=${payload.status} code=${payload.code}`,
    )
  }
  ...
  if (
    returnedContainerNumber !== null &&
    normalizeContainerNumber(returnedContainerNumber) !==
      normalizeContainerNumber(command.containerNumber)
  ) {
    errors.push(
      `ONE search returned mismatched container ${normalizeContainerNumber(returnedContainerNumber)}`,
    )
  }
...
function validateVoyageListResult(command: {
  ...
  readonly meta: OneEndpointMeta
  readonly parseError: string | null
  readonly parsed: OneVoyageListResponse | null
} {
  ...
  if (!isSuccessfulCarrierEnvelope(schemaResult.data)) {
    errors.push(
      `ONE voyage-list returned unsuccessful carrier envelope status=${schemaResult.data.status} code=${schemaResult.data.code}`,
    )
  }
...
function validateCopEventsResult(command: {
  ...
  readonly meta: OneEndpointMeta
  readonly parseError: string | null
  readonly parsed: OneCopEventsResponse | null
} {
  ...
  if (!isSuccessfulCarrierEnvelope(schemaResult.data)) {
    errors.push(
      `ONE cop-events returned unsuccessful carrier envelope status=${schemaResult.data.status} code=${schemaResult.data.code}`,
    )
  }
...
function buildSkippedMeta(reason: string): OneEndpointMeta {
...
async function fetchVoyageListWithGracefulFailure(
  bookingNo: string,
): Promise<OneSecondaryFetchResult> {
  try {
...
function buildRawSnapshot(command: {
  ...
  readonly searchPayload: unknown | null
  readonly voyageListPayload: unknown | null
  readonly copEventsPayload: unknown | null
...
export async function fetchOneStatus(containerNumber: string): Promise<FetchResult> {
