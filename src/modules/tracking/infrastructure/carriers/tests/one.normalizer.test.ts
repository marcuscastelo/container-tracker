...
describe('normalizeOneSnapshot', () => {
  ...
  it('keeps the derived shipment state IN_TRANSIT and exposes voyage/transshipment blocks for Karachi to Santos', () => {
    ...
    expect(transshipmentBlocks[0]?.block.previousVesselName).toBe('CARL SCHULTE')
    expect(transshipmentBlocks[0]?.block.nextVesselName).toBe('PARANA EXPRESS')
    expect(transshipmentBlocks[0]?.block.handoffDisplayMode).toBe('FULL')
