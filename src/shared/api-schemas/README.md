...
- Keep single source of truth for API request/response shapes under `src/shared/api-schemas`.
- File naming: `feature.schemas.ts` or `route.schemas.ts` (e.g., `processes.schemas.ts`).
- Export both zod schemas and TypeScript types (via `export type X = z.infer<typeof XSchema>`).
