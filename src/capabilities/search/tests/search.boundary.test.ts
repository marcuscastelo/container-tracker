...
const SEARCH_CAPABILITY_ROOT = path.resolve(process.cwd(), 'src/capabilities/search')
...
describe('search capability boundary guard', () => {
  it('forbids imports from modules/*/domain within src/capabilities/search/**', async () => {
    const sourceFiles = await listSourceFiles(SEARCH_CAPABILITY_ROOT)
