...
export async function fetchStatus(container: string): Promise<{ parsedStatus?: Record<string, unknown>; raw?: string }>{
  ...
  try {
    ...
    resp = await axios.post(url, payload, { headers, responseType: 'arraybuffer' })
  } catch (err: any) {
  ...
  try {
    ...
    const contentEncoding = (resp.headers && (resp.headers['content-encoding'] as string | undefined)) || null
    try {
      const zlib = await import('zlib')
      if (contentEncoding === 'gzip' || (buf.length >= 2 && buf[0] === 0x1f && buf[1] === 0x8b)) {
        text = zlib.gunzipSync(buf).toString('utf8')
      } else if (contentEncoding === 'deflate' || (buf.length >= 2 && buf[0] === 0x78 && (buf[1] === 0x9c || buf[1] === 0x01 || buf[1] === 0xda))) {
