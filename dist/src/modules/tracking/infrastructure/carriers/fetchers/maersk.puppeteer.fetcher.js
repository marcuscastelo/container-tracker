...
function isTrackingApiResponseUrl(url) {
    const normalized = url.toLowerCase();
    return normalized.includes('api.maersk.com') && normalized.includes('/synergy/tracking');
}
function scoreTrackingCaptureCandidate(url, body, container) {
...
function toCaptureCookie(cookie) {
    if (!isRecord(cookie))
        return {};
    return {
...
function shouldWriteDevtoolsDiagnostics() {
...
export function createMaerskCaptureService() {
    return {
        async capture(command) {
            ...
            console.log(`[maersk-refresh] Using Chrome at: ${browserResolution.executablePath} (source: ${browserResolution.source})`);
            try {
                    ...
                    try {
                        ...
                        if (responseUrl &&
                            ...
                            try {
                                ...
                                const candidateScore = scoreTrackingCaptureCandidate(responseUrl, body, command.container);
                ...
                if (!captureState.data) {
                    try {
                        await page.waitForFunction(() => typeof window !== 'undefined' &&
                            ...
                            window.bmak &&
                            // @ts-expect-error dynamic access to anti-bot runtime
                            typeof window.bmak.get_telemetry === 'function', { timeout: 10000 });
                        await delay(2000);
                    }
                    catch {
