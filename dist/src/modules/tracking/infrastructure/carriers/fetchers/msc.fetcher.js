 ...
 */
export async function fetchMscStatus(containerNumber) {
    ...
    const contentEncoding = resp.headers?.['content-encoding'] ?? undefined;
    const text = decodeResponseBuffer(Buffer.from(resp.data), typeof contentEncoding === 'string' ? contentEncoding : null);
...
// Helpers
// ---------------------------------------------------------------------------
function decodeResponseBuffer(buf, contentEncoding) {
    try {
