...
import { publishAgentControlPublicSnapshot, refreshAgentControlPublicLogs, } from "../control-core/public-control-files.js";
...
import * as releaseManager from "../release-manager.js";
import { readReleaseState, withRecordedFailure, writeReleaseState } from "../release-state.js";
import { EXIT_CONFIG_ERROR, EXIT_FATAL, EXIT_OK, resolveSupervisorExitAction, } from "../runtime/lifecycle-exit-codes.js";
import { resolveAgentPublicBackendStatePath, resolveAgentPublicLogsPath, resolveAgentPublicStatePath, } from "../runtime/paths.js";
import { readRuntimeHealth } from "../runtime-health.js";
import { ensureAgentPathLayout, resolveAgentPathLayout } from "../runtime-paths.js";
import { clearSupervisorControl } from "../supervisor-control.js";
import { resolveAutomaticUpdateChecksMode } from "../update-checks.js";
import { createRotatingChunkWriter } from "./runtime-stdio-log-writer.js";
...
let publicArtifactPublisher = null;
function sleep(ms) {
...
async function publishSupervisorPublicSnapshot(layout) {
...
function refreshPublicLogArtifacts(layout) {
    if (process.platform !== 'linux') {
        return;
    }
    try {
...
function createPublicArtifactPublisher(layout) {
    let timer = null;
    return {
        requestRefresh() {
            if (timer) {
...
function requestPublicArtifactRefresh() {
    publicArtifactPublisher?.requestRefresh();
}
function startPublicSnapshotRefreshLoop(layout) {
    if (process.platform !== 'linux') {
        return null;
    }
...
function rotateLogIfNeeded(logPath, maxSizeBytes) {
...
function appendSupervisorLog(logsDir, message) {
...
function resolveFallbackRuntimeEntrypoint(scriptDir) {
    const candidates = [
...
export function resolveRuntimeExecArgv(scriptPath) {
...
function readAgentVersion(startDir) {
...
async function runChildWithHealthGate(command) {
    ...
    appendSupervisorLog(command.logsDir, `started runtime pid=${child.pid ?? 'unknown'} version=${command.expectedVersion} entrypoint=${command.scriptPath}`);
...
async function runChildWithoutHealthGate(command) {
    ...
    appendSupervisorLog(command.logsDir, `started runtime pid=${child.pid ?? 'unknown'} version=${command.version} entrypoint=${command.scriptPath}`);
...
export async function runSupervisorMain() {
    ...
    const startupTimeoutMs = resolveNumberEnv(normalizeOptionalEnv(process.env.AGENT_UPDATE_STARTUP_TIMEOUT_MS), DEFAULT_STARTUP_TIMEOUT_MS);
    const HEALTH_GRACE_MS = resolveNumberEnv(normalizeOptionalEnv(process.env.AGENT_UPDATE_HEALTH_GRACE_MS) ??
        normalizeOptionalEnv(process.env.AGENT_UPDATE_STABILITY_WINDOW_MS), DEFAULT_HEALTH_GRACE_MS);
    const crashLoopWindowMs = resolveNumberEnv(normalizeOptionalEnv(process.env.AGENT_UPDATE_CRASH_LOOP_WINDOW_MS), DEFAULT_CRASH_LOOP_WINDOW_MS);
    const crashLoopThreshold = resolveNumberEnv(normalizeOptionalEnv(process.env.AGENT_UPDATE_CRASH_LOOP_THRESHOLD), DEFAULT_CRASH_LOOP_THRESHOLD);
    const MAX_ACTIVATION_FAILURES = resolveNumberEnv(normalizeOptionalEnv(process.env.AGENT_UPDATE_MAX_ACTIVATION_FAILURES), DEFAULT_MAX_ACTIVATION_FAILURES);
    ...
    if (updateManifestChecksDisabled) {
        if (updateChecksMode.reason === 'EXPLICIT_DISABLE_FLAG') {
            appendSupervisorLog(layout.logsDir, `automatic update checks disabled by AGENT_DISABLE_AUTOMATIC_UPDATE_CHECKS; forcing fallback runtime selection (configured channel=${updateChecksMode.configuredChannel ?? 'unknown'})`);
        }
        else {
            appendSupervisorLog(layout.logsDir, 'automatic update checks disabled; forcing fallback runtime selection');
        }
    }
    ...
    for (;;) {
        ...
        if (!updateManifestChecksDisabled &&
            ...
            try {
                ...
                const failureTracked = failedTargetVersion === null
                    ? null
                    : withRecordedFailure({
                        state,
                ...
                if (failureTracked?.newlyBlocked && failedTargetVersion) {
                    appendSupervisorLog(layout.logsDir, `[update] version ${failedTargetVersion} blocked after ${failureTracked.activationFailuresForVersion} activation failures`);
                }
                ...
                writeReleaseState(layout.releaseStatePath, state);
                appendSupervisorLog(layout.logsDir, `activation failed target=${failedTargetVersion ?? 'unknown'} rollback=${rollbackVersion} error="${errorMessage}"`);
                appendPendingActivityEvents(layout.pendingActivityPath, [
                    {
        ...
        appendSupervisorLog(layout.logsDir, `runtime selected source=${runtimeSelection.source} version=${runtimeSelection.version} entrypoint=${runtimeSelection.entrypointPath} activation_state=${state.activation_state}`);
        await publishSupervisorPublicSnapshot(layout);
        clearSupervisorControl(layout.supervisorControlPath);
        const requireHealthGate = state.activation_state === 'verifying' &&
        ...
        const runResult = requireHealthGate
            ? await runChildWithHealthGate({
                ...
                onStabilityConfirmed() {
                    ...
                    appendSupervisorLog(layout.logsDir, `activation confirmed for version=${currentState.target_version}`);
                    void publishSupervisorPublicSnapshot(layout);
                },
            })
        ...
        const failedReleaseRuntimeWithoutHealthGate = !requireHealthGate && runtimeSelection.source === 'release' && runResult.exitCode !== 0;
        ...
        const shouldRollbackFromHealthGate = requireHealthGate &&
        ...
        if (shouldRollbackFromHealthGate && refreshedState.target_version) {
            ...
            if (failureTracked.newlyBlocked) {
                appendSupervisorLog(layout.logsDir, `[update] version ${refreshedState.target_version} blocked after ${failureTracked.activationFailuresForVersion} activation failures`);
            }
            const rollbackVersion = selectRollbackVersion({
                releasesDir: layout.releasesDir,
            ...
            consecutiveReleaseRuntimeFailures = 0;
            appendSupervisorLog(layout.logsDir, `rollback executed to ${rollbackVersion} after health gate failure (target=${refreshedState.target_version})`);
            appendPendingActivityEvents(layout.pendingActivityPath, [
                {
                    type: 'UPDATE_APPLY_FAILED',
        ...
        if (failedReleaseRuntimeWithoutHealthGate) {
            appendSupervisorLog(layout.logsDir, `release runtime failure count=${consecutiveReleaseRuntimeFailures}/${crashLoopThreshold} version=${runtimeSelection.version}`);
        }
        const shouldFallbackAfterReleaseCrashLoop = failedReleaseRuntimeWithoutHealthGate &&
        ...
        if (shouldFallbackAfterReleaseCrashLoop) {
            ...
            if (failureTracked.newlyBlocked) {
                appendSupervisorLog(layout.logsDir, `[update] version ${runtimeSelection.version} blocked after ${failureTracked.activationFailuresForVersion} activation failures`);
            }
            ...
            appendSupervisorLog(layout.logsDir, `release crash loop guard switched runtime to fallback version=${fallbackVersion} from release=${runtimeSelection.version}`);
            appendPendingActivityEvents(layout.pendingActivityPath, [
                {
                    type: 'UPDATE_APPLY_FAILED',
        ...
        if (exitAction === 'stop-config-error') {
            appendSupervisorLog(layout.logsDir, 'runtime exited with configuration error (code=50), supervisor stopping without restart');
            supervisorExitCode = EXIT_CONFIG_ERROR;
            break;
        }
        appendSupervisorLog(layout.logsDir, `runtime exited with code=${runResult.exitCode ?? 'unknown'}, restarting after backoff`);
        await sleep(RESTART_BACKOFF_MS);
    }
    if (publicSnapshotRefreshLoop) {
        clearInterval(publicSnapshotRefreshLoop);
