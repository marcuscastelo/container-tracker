...
import { acknowledgeRemoteCommand, applyRemoteCommand, syncAgentControlState, } from '@agent/control-core/agent-control-core';
...
import { fetchCmaCgmStatus } from "../../../../src/modules/tracking/infrastructure/carriers/fetchers/cmacgm.fetcher.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { createMaerskCaptureService } from "../../../../src/modules/tracking/infrastructure/carriers/fetchers/maersk.puppeteer.fetcher.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { fetchMscStatus } from "../../../../src/modules/tracking/infrastructure/carriers/fetchers/msc.fetcher.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { fetchOneStatus } from "../../../../src/modules/tracking/infrastructure/carriers/fetchers/one.fetcher.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { fetchPilStatus } from "../../../../src/modules/tracking/infrastructure/carriers/fetchers/pil.fetcher.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { subscribeSyncRequestsByTenant } from "../../../../src/shared/supabase/sync-requests.realtime.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { createAgentScheduler } from "../agent.scheduler.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { computeBackoffDelayMs } from "../backoff.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { createAgentLogForwarder } from "../log-forwarder.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { drainPendingActivityEvents } from "../pending-activity.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { resolveAgentPlatformKey } from "../platform/platform.adapter.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { readReleaseState, writeReleaseState } from "../release-state.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { writeRuntimeHealth } from "../runtime-health.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
...
import * as runtimePaths from "../runtime-paths.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
// biome-ignore lint/performance/noNamespaceImport: Runtime keeps grouped imports stable to avoid formatter wrapping regressions.
import * as supervisorControl from "../supervisor-control.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { resolveAutomaticUpdateChecksMode } from "../update-checks.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { fetchUpdateManifest, stageReleaseFromManifest } from "../updater.core.js";
// biome-ignore lint/style/noRestrictedImports: Agent runtime uses Node --experimental-strip-types with direct .ts imports.
import { EXIT_FATAL, EXIT_UPDATE_RESTART } from "./lifecycle-exit-codes.js";
...
import { resolveAgentPublicBackendStatePath, resolveAgentPublicStatePath } from "./paths.js";
...
function hasCauseProperty(value) {
    return typeof value === 'object' && value !== null && 'cause' in value;
}
function toUnknownMessage(value) {
...
function toErrorMessage(error, secrets = []) {
...
function getEnvValue(key, fromFile) {
...
function toControlRuntimeConfig(config) {
    return {
...
function toRuntimeConfigFromControlConfig(config) {
...
async function syncControlStateAndPersistPublicState(command) {
    const syncCommand = typeof command.forceRemoteFetch === 'boolean'
        ? {
...
function detectRuntimePlaceholderKeys(config) {
    const keys = [];
...
function detectBootstrapPlaceholderKeys(config) {
    const keys = [];
    if (resolveUrlHost(config.BACKEND_URL) === PLACEHOLDER_BACKEND_HOST) {
        keys.push('BACKEND_URL');
    }
...
function parseRuntimeConfigFromFile(filePath) {
    ...
    if (!parsed.success) {
        console.warn(`[agent] config.env is invalid, switching to bootstrap mode: ${parsed.error.message}`);
        return null;
    }
    const placeholderKeys = detectRuntimePlaceholderKeys(parsed.data);
    if (placeholderKeys.length > 0) {
        console.warn(`[agent] config.env contains placeholder values (${placeholderKeys.join(', ')}), switching to bootstrap mode`);
        return null;
    }
    return parsed.data;
}
function parseBootstrapConfigFromFile(filePath) {
    if (!fs.existsSync(filePath))
        return null;
    const loaded = loadEnvFile(filePath);
    ...
    if (placeholderKeys.length > 0) {
        throw new Error(`invalid bootstrap.env: placeholder values detected in ${placeholderKeys.join(', ')}`);
    }
    return {
        config: parsed.data,
        raw: loaded.raw,
...
function resolveMachineFingerprint(hostname) {
...
function serializeConsumedBootstrap(bootstrapConfig) {
    return [
...
function persistConfigFile(configPath, config) {
...
function consumeBootstrapFile(command) {
    ...
    const safeContent = consumedContent === command.bootstrapRaw
...
async function resolveRuntimeConfigWithBootstrap(paths) {
    let enrollAttempt = 0;
    for (;;) {
        ...
        try {
            ...
            console.error(`[agent] bootstrap configuration unavailable: ${toErrorMessage(error)} (retry in ${Math.round(delayMs / 1000)}s)`);
            enrollAttempt += 1;
            await sleep(delayMs);
            continue;
        }
        ...
        try {
            ...
            const delayMs = computeBackoffDelayMs(enrollAttempt);
            console.error(`[agent] enrollment failed (attempt=${enrollAttempt + 1}): ${toErrorMessage(error, secrets)} (retry in ${Math.round(delayMs / 1000)}s)`);
            enrollAttempt += 1;
            await sleep(delayMs);
        }
    }
...
function buildHeaders(config, contentType) {
...
function resolveAgentCapabilities(config) {
    if (config.MAERSK_ENABLED)
        return ['msc', 'cmacgm', 'pil', 'one', 'maersk'];
...
function resolveSupervisorPaths(dataDir) {
    const healthPath = normalizeOptionalEnv(process.env.AGENT_SUPERVISOR_HEALTH_PATH) ??
        path.join(dataDir, 'runtime-health.json');
    const controlPath = normalizeOptionalEnv(process.env.AGENT_SUPERVISOR_CONTROL_PATH) ??
        path.join(dataDir, 'supervisor-control.json');
    const pendingActivityPath = normalizeOptionalEnv(process.env.AGENT_PENDING_ACTIVITY_PATH) ??
...
function toRuntimeActivityFromPending(event) {
    return {
        type: event.type,
        message: event.message,
        severity: event.severity,
...
async function fetchTargets(config, limit, recoverOwnedLeases = false) {
...
async function performScrapeTarget(config, target) {
...
async function scrapeTarget(config, target) {
    try {
...
async function ingestSnapshot(config, target, scrape, agentVersion) {
    ...
    if (response.status === 422) {
        ...
        if (parsed.data.snapshot_id) {
            console.warn(`[agent] backend marked ${target.ref} as failed after persisting snapshot ${parsed.data.snapshot_id}: ${parsed.data.error}`);
        }
        else {
    ...
    const summary = newObservationsCount === undefined
        ? ''
...
async function processTarget(config, target, agentVersion) {
    ...
    try {
        ...
        console.warn(`[agent] target ${target.sync_request_id} will be available again after lease expiration`);
        return {
            kind: 'failed',
            durationMs: Math.max(0, Date.now() - startedAtMs),
...
async function runOnce(config, agentVersion, state, reason) {
...
function buildRealtimeSignalFingerprint(signal) {
...
function buildRealtimeDegradedMessage(status) {
...
function toRealtimeSignal(status) {
    const metadata = {
...
function subscribeToRealtimeIfConfigured(command) {
    ...
    try {
        ...
        let lastLoggedSignalFingerprint = null;
        const supabaseRealtime = createClient(command.config.SUPABASE_URL, command.config.SUPABASE_ANON_KEY, {
            db: {
                schema: 'public',
            },
            auth: {
...
async function main() {
    ...
    console.log(`[agent] started (tenant=${runtimeConfig.TENANT_ID}, agent=${runtimeConfig.AGENT_ID}, interval=${runtimeConfig.INTERVAL_SEC}s)`);
    if (updateManifestChecksDisabled) {
        if (updateChecksMode.reason === 'EXPLICIT_DISABLE_FLAG') {
            console.log(`[agent:update] manifest checks disabled by AGENT_DISABLE_AUTOMATIC_UPDATE_CHECKS; configured channel=${updateChecksMode.configuredChannel ?? 'unknown'}; using current runtime only`);
        }
        else {
            console.log('[agent:update] manifest checks disabled (AGENT_UPDATE_MANIFEST_CHANNEL=disabled); using current runtime only');
        }
    }
    const runtimeState = {
        realtimeState: runtimeConfig.SUPABASE_URL && runtimeConfig.SUPABASE_ANON_KEY ? 'CONNECTING' : 'DISCONNECTED',
    ...
    logForwarder.start();
    const pendingActivities = drainPendingActivityEvents(supervisorPaths.pendingActivityPath).map(toRuntimeActivityFromPending);
    ...
    runtimeConfig = toRuntimeConfigFromControlConfig(controlSync.effectiveConfig);
    let lastRealtimeSignalFingerprint = null;
    ...
    scheduler = createAgentScheduler({
        ...
        runCycle: async (reason) => {
            ...
            const shouldRunUpdateCheck = !updateManifestChecksDisabled &&
    ...
    realtimeSubscription = subscribeToRealtimeIfConfigured({
        ...
        onRealtimeStateChange(signal) {
            ...
            if (lastRealtimeSignalFingerprint === fingerprint)
                return;
