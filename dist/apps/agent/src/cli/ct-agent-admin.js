...
import { AgentControlBackendStateSchema, AgentControlBackendUpdateResultSchema, AgentControlCommandResultSchema, AgentControlLogChannelSchema, AgentControlLogsResponseSchema, AgentControlPathsSchema, AgentOperationalSnapshotSchema, AgentReleaseInventorySchema, } from '@agent/control-core/contracts';
import { writeAgentControlPublicBackendState } from '@agent/control-core/public-control-files';
import { readAgentControlPublicState, writeAgentControlPublicState, } from '@agent/control-core/public-control-state';
import { EXIT_CONFIG_ERROR, EXIT_FATAL, EXIT_OK } from '@agent/runtime/lifecycle-exit-codes';
import { resolveAgentPublicBackendStatePath, resolveAgentPublicStatePath, } from '@agent/runtime/paths';
...
function isCtAgentAdminCommand(value) {
...
function printUsage(print) {
    print(['Usage: ct-agent-admin <command> [json]', `Commands: ${ctAgentAdminCommands.join(', ')}`].join('\n'));
}
...
function persistPublicState(service, snapshot) {
...
export function resolveCtAgentAdminLayout() {
...
export async function runCtAgentAdmin(command) {
    ...
    try {
        ...
        if (subcommand === 'change-channel') {
            ...
            const result = AgentControlCommandResultSchema.parse(await service.changeChannel(input.channel));
            await persistPublicState(service, result.snapshot);
            print(JSON.stringify(result, null, 2));
            return EXIT_OK;
        }
        if (subcommand === 'set-blocked-versions') {
            ...
            const result = AgentControlCommandResultSchema.parse(await service.setBlockedVersions(input.versions));
            await persistPublicState(service, result.snapshot);
            print(JSON.stringify(result, null, 2));
            return EXIT_OK;
        }
        ...
        if (subcommand === 'activate-release') {
            ...
            const result = AgentControlCommandResultSchema.parse(await service.activateRelease(input.version));
            await persistPublicState(service, result.snapshot);
            print(JSON.stringify(result, null, 2));
            return EXIT_OK;
        }
        ...
        if (subcommand === 'set-backend-url') {
            ...
            const result = AgentControlBackendUpdateResultSchema.parse(await service.setBackendUrl(input.backendUrl));
...
if (isMainModule()) {
    if (isRunningAsRoot()) {
        void runCtAgentAdmin({ argv: process.argv }).then((exitCode) => {
