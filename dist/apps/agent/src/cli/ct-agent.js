...
import { EXIT_CONFIG_ERROR, EXIT_FATAL, EXIT_OK } from "../runtime/lifecycle-exit-codes.js";
// biome-ignore lint/style/noRestrictedImports: CLI runtime resolves direct .ts imports for bundled agent execution.
import { readRuntimeHealth } from "../runtime-health.js";
// biome-ignore lint/style/noRestrictedImports: CLI runtime resolves direct .ts imports for bundled agent execution.
import { resolveAgentPathLayout } from "../runtime-paths.js";
...
const ctAgentCommands = ['status', 'logs', 'restart', 'update-status', 'enroll'];
...
function resolveAgentVersion(scriptDir) {
...
async function enrollRuntime(command) {
    const response = await command.fetchImpl(`${command.bootstrapConfig.BACKEND_URL}/api/agent/enroll`, {
        method: 'POST',
        headers: {
...
function isCtAgentCommand(value) {
...
async function runLogsCommand(runCommand) {
    const journalArgs = [
        '-u',
        AGENT_SERVICE_NAME,
        '-n',
    ...
    try {
        ...
        console.warn(`[ct-agent] journalctl exited with code ${journalExitCode}; falling back to local logs`);
    }
    catch (error) {
    ...
    if (localLogs.length === 0) {
        console.error(`[ct-agent] local log files not found: ${outLogPath}, ${errLogPath}, supervisor.log`);
        return EXIT_FATAL;
    }
    try {
...
async function runRestartCommand(runCommand) {
    try {
...
async function runUpdateStatusCommand() {
    const layout = resolveAgentPathLayout();
...
async function runEnrollCommand(fetchImpl) {
    const layout = resolveAgentPathLayout();
    const existingConfig = parseRuntimeConfigFromFile(layout.configPath);
    if (existingConfig) {
        console.log(`[ct-agent] already enrolled (tenant=${existingConfig.TENANT_ID}, agent=${existingConfig.AGENT_ID})`);
        return EXIT_OK;
    }
    ...
    try {
        ...
        console.log(`[ct-agent] enroll successful (tenant=${runtimeConfig.TENANT_ID}, agent=${runtimeConfig.AGENT_ID})`);
        return EXIT_OK;
    }
    catch (error) {
...
if (isMainModule()) {
    void runCtAgent({ argv: process.argv }).then((exitCode) => {
