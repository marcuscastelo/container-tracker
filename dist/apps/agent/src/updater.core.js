...
import { resolveAgentPlatformKey, resolvePlatformAdapter } from "./platform/platform.adapter.js";
// biome-ignore lint/style/noRestrictedImports: Shared updater runtime resolves direct .ts imports in release artifacts.
...
// biome-ignore lint/style/noRestrictedImports: Shared updater runtime resolves direct .ts imports in release artifacts.
import { hasBlockedVersion } from "./release-state.js";
...
function inferArchiveKind(downloadUrl) {
...
function buildAuthHeaders(command, includeContentType) {
    ...
    if (typeof command.updateChannelOverride === 'string' &&
...
function createNoUpdateManifest(channel) {
    return {
        version: '0.0.0',
        download_url: null,
        checksum: null,
...
export async function fetchUpdateManifest(command, fetchImpl = fetch) {
    const platform = command.platform ?? resolveAgentPlatformKey();
    const manifestUrl = `${command.backendUrl}/api/agent/update-manifest`;
    console.log(`[agent:update] checking manifest url=${manifestUrl} platform=${platform} agent=${command.agentId}`);
    ...
    if (response.status === 204) {
        ...
        console.warn(`[agent:update] manifest status=204 (No Content) status=${status} channel=${channel} reason=${reason} platform=${platform}; backend reported manifest_unavailable for this agent/channel`);
        return createNoUpdateManifest('stable');
    }
    if (!response.ok) {
        const details = await response.text().catch(() => '');
        console.error(`[agent:update] manifest request failed status=${response.status} platform=${platform}`);
    ...
    console.log(`[agent:update] manifest status=${response.status} channel=${parsed.data.channel} version=${parsed.data.version} update_available=${parsed.data.update_available} desired=${parsed.data.desired_version ?? 'none'} current=${parsed.data.current_version} update_ready=${parsed.data.update_ready_version ?? 'none'}`);
...
function removeReleaseDirectoryIfPresent(releaseDir) {
    if (!fs.existsSync(releaseDir)) {
        return;
    }
    fs.rmSync(releaseDir, { recursive: true, force: true });
}
function removeReleaseDirectoryWhenEntrypointMissing(releaseDir) {
    if (!fs.existsSync(releaseDir)) {
        return false;
    }
...
function findReleaseDirFromExtractedRoot(extractedRoot) {
...
function prepareReleaseDirectory(command) {
    ...
    const stagingDir = path.join(command.layout.releasesDir, `.staging-${sanitizedVersion}-${Date.now().toString(36)}`);
    ...
    if (!extractedReleaseDir) {
        ...
        throw new Error(`release archive does not contain a valid entrypoint for version ${command.version}`);
    }
    removeReleaseDirectoryWhenEntrypointMissing(releaseDir);
...
export async function stageReleaseFromManifest(command) {
    ...
    if (!command.manifest.download_url || !command.manifest.checksum) {
        throw new Error(`update manifest for ${command.manifest.version} is missing download URL or checksum`);
    }
    const releaseDir = releaseManager.resolveReleaseDir(command.layout.releasesDir, command.manifest.version);
    ...
    if (!response.ok) {
        const details = await response.text().catch(() => '');
        throw new Error(`release download failed for ${command.manifest.version} (${response.status}): ${details}`);
    }
    ...
    if (observedChecksum !== expectedChecksum) {
        throw new Error(`checksum mismatch for ${command.manifest.version}: expected ${expectedChecksum}, got ${observedChecksum}`);
    }
