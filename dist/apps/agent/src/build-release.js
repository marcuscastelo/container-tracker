...
const NODE_WINDOWS_SHA256_BY_VERSION = {
...
const RELEASE_APP_REQUIRED_TOP_LEVEL_ENTRIES = ['dist', 'node_modules', 'package.json'];
...
async function resolveBootstrapEnvConfig(repoRoot) {
    ...
    if (!backendUrl || !installerToken) {
        throw new Error('missing BACKEND_URL and/or INSTALLER_TOKEN in process env or repository .env for bootstrap.env generation');
    }
    const hostnameFallback = normalizeEnvValue(process.env.COMPUTERNAME) ??
...
function serializeBootstrapEnv(config) {
    const lines = [
...
async function runPnpmCommand(args, cwd) {
    if (process.platform === 'win32') {
        ...
        try {
            ...
            lastError = error;
            console.warn(`[agent:release] failed to run "pnpm.cmd" via cmd.exe (${toErrorMessage(error)}), trying direct binary...`);
        }
        try {
...
async function runPnpmDeployWithLegacyFallback(command) {
    ...
    try {
        ...
        console.warn(`[agent:release] pnpm deploy with --legacy failed (${toErrorMessage(legacyError)}); retrying without --legacy`);
    }
    try {
        await runPnpmCommand(command.args, command.cwd);
    }
    catch (defaultError) {
        throw new Error(`pnpm deploy failed both with and without --legacy (${toErrorMessage(defaultError)})`);
    }
}
async function extractZip(zipPath, destinationDir, cwd) {
    if (process.platform === 'win32') {
        try {
...
async function verifyNodeRuntimeChecksum(command) {
    ...
    if (!expectedSha256) {
        console.warn(`[agent:release] node checksum verification skipped: no pinned SHA-256 for ${command.nodeVersion}`);
        return;
    }
    const observedSha256 = await computeFileSha256(command.nodeZipPath);
    if (observedSha256 !== expectedSha256) {
        throw new Error(`node runtime checksum mismatch for ${command.nodeVersion} (expected ${expectedSha256}, got ${observedSha256})`);
    }
    console.log(`[agent:release] verified node runtime SHA-256 for ${command.nodeVersion}`);
}
async function findExtractedNodeDirectory(extractDir) {
...
async function writeAgentEntrypointShims(distDir) {
    ...
    await fs.writeFile(agentShimPath, `${shimBanner}${registerImport}await import('./apps/agent/src/supervisor.js')\n`, 'utf8');
    await fs.writeFile(updaterShimPath, `${shimBanner}${registerImport}await import('./apps/agent/src/updater.js')\n`, 'utf8');
}
...
function remapAbsoluteRuntimeSymlinkTarget(command) {
    if (isInsideDirectory(command.sourceNodeModulesDir, command.resolvedTargetPath)) {
        const relativeFromSourceRoot = path.relative(command.sourceNodeModulesDir, command.resolvedTargetPath);
...
function normalizeInstallerCommandLine(line) {
    return line.replaceAll('""', '"').toLowerCase();
}
export function collectInstallerTaskRegistrationErrors(installerContentRaw) {
    const taskRegistrationLines = installerContentRaw
        .split(/\r?\n/)
        .map(normalizeInstallerCommandLine)
        .filter((line) => (line.includes('filename: "schtasks.exe"') && line.includes('/create')) ||
    ...
    const errors = [];
    const hasAgentCreate = taskRegistrationLines.some((line) => line.includes('powershell.exe') &&
        line.includes('-windowstyle hidden') &&
        line.includes('run-supervisor.ps1'));
    const hasUpdaterCreate = taskRegistrationLines.some((line) => line.includes('powershell.exe') &&
        line.includes('-windowstyle hidden') &&
    ...
    const usesLegacyAgentTrayHost = taskRegistrationLines.some((line) => line.includes('agent-tray-host.ps1'));
    ...
    const launchesRuntimeShimDirectly = taskRegistrationLines.some((line) => line.includes('app\\dist\\agent.js'));
    ...
    for (const line of taskRegistrationLines) {
        ...
        if (usesSchtasksCreate) {
            if (!line.includes('/sc onlogon') || !line.includes('/it') || !line.includes('/rl limited')) {
                errors.push('installer.iss schtasks registrations must include /SC ONLOGON, /IT and /RL LIMITED');
                break;
            }
        ...
        if (!line.includes('new-scheduledtasktrigger -atlogon') ||
            !line.includes('logontype interactive') ||
            !line.includes('runlevel limited')) {
            errors.push('installer.iss Register-ScheduledTask registrations must use AtLogOn, Interactive logon and RunLevel Limited');
            break;
        }
        if (line.includes('-userid system') ||
...
async function resolvePackagePathFromTargetPnpmStore(command) {
    ...
    const packageDirFromStore = await resolvePackageDirFromPnpmStore(targetPnpmStoreDir, packagePath.packageName);
...
async function resolveRuntimeDependencyPackage(command) {
    ...
    const packageDirFromStore = await resolvePackageDirFromPnpmStore(command.pnpmStoreDir, command.packageName);
    if (packageDirFromStore) {
        const storeDepId = extractPnpmDepId(command.pnpmStoreDir, packageDirFromStore);
        if (!storeDepId) {
            throw new Error(`runtime dependency package "${command.packageName}" is outside pnpm store: ${packageDirFromStore}`);
        }
        return {
            packageName: command.packageName,
...
async function collectRuntimePnpmDependencyIds(command) {
    ...
    for (const packageName of command.directDependencies) {
        directResolutions.push(await resolveRuntimeDependencyPackage({
...
async function pruneReleaseAppTopLevelEntries(releaseAppDir) {
    const keepEntries = new Set(RELEASE_APP_REQUIRED_TOP_LEVEL_ENTRIES);
...
async function pruneRuntimeNodeModules(releaseAppDir, runtimeSnapshot) {
    ...
    const resolvedSnapshot = runtimeSnapshot ??
    ...
    const keepTopLevelEntries = new Set(NODE_MODULES_METADATA_ENTRIES);
    ...
    const topLevelLinkTargets = new Map();
    ...
    for (const resolution of directResolutions) {
        topLevelLinkTargets.set(resolution.packageName, path.join(pnpmStoreDir, resolution.depId, 'node_modules', ...toPackageNameSegments(resolution.packageName)));
    }
    ...
    console.log(`[agent:release] pruned runtime node_modules to ${dependencyIds.size} package snapshots and ${topLevelLinkTargets.size} top-level links`);
}
async function pruneRuntimeNodeModulesArtifacts(releaseAppDir) {
    const nodeModulesDir = path.join(releaseAppDir, 'node_modules');
...
async function ensureAgentRuntimeDependenciesInReleaseApp(command) {
    ...
    for (const resolution of directResolutions) {
        ...
        const storePackagePath = path.join(targetPnpmStoreDir, resolution.depId, 'node_modules', ...toPackageNameSegments(resolution.packageName));
        const storePackageJson = path.join(storePackagePath, 'package.json');
        if (!(await pathExists(storePackageJson))) {
            throw new Error(`runtime dependency missing after sync: ${resolution.packageName} (${storePackagePath})`);
        }
        const relativeLinkTarget = path.relative(path.dirname(topLevelEntryPath), storePackagePath);
        await fs.symlink(relativeLinkTarget, topLevelEntryPath, symlinkType);
    }
    ...
    const unpackedRuntimeDependencyCount = directResolutions.filter((resolution) => resolution.depId === null).length;
    const unpackedSuffix = unpackedRuntimeDependencyCount > 0
        ? ` and copied ${unpackedRuntimeDependencyCount} unpacked direct runtime package(s)`
        : '';
    console.log(`[agent:release] synced ${dependencyIds.size} runtime package snapshots from workspace cache${unpackedSuffix}`);
...
async function pruneReleaseAppForRuntime(releaseAppDir, runtimeSnapshot) {
...
async function runPreflightChecks(command) {
    ...
    for (const packageName of REQUIRED_RUNTIME_DEPENDENCY_PACKAGES) {
        const topLevelPackageJson = path.join(resolvePackageEntryPath(releaseAppNodeModulesDir, packageName), 'package.json');
        if (await pathExists(topLevelPackageJson)) {
            continue;
        }
        const packageDirFromStore = await resolvePackageDirFromPnpmStore(releasePnpmStoreDir, packageName);
    ...
    if (await pathExists(command.installerFilePath)) {
        ...
        const hasX64Mode = installerContentRaw.includes('ArchitecturesAllowed=x64') &&
        ...
        if (!installerContentRaw.includes('run-supervisor.ps1') ||
    ...
    if (await pathExists(command.canonicalRuntimePathsSourcePath)) {
        const canonicalRuntimePathsSource = await fs.readFile(command.canonicalRuntimePathsSourcePath, 'utf8');
        if (!canonicalRuntimePathsSource.includes('resolveAgentDataDir') ||
            !canonicalRuntimePathsSource.includes('/var/lib/container-tracker-agent') ||
            !canonicalRuntimePathsSource.includes('.agent-runtime')) {
            errors.push('runtime/paths.ts must provide Linux data-dir defaults (/var/lib/container-tracker-agent with .agent-runtime fallback)');
        }
    }
    else {
    ...
    if (await pathExists(command.windowsAdapterSourcePath)) {
        ...
        if (!windowsAdapterSource.includes('DEFAULT_DATA_DIR_NAME') ||
            !windowsAdapterSource.includes('ContainerTracker') ||
            !windowsAdapterSource.includes('LOCALAPPDATA')) {
            errors.push('windows.adapter.ts must resolve fallback paths under LOCALAPPDATA\\ContainerTracker');
        }
    }
    else {
    ...
    console.log(`[agent:release] release directory: ${path.relative(command.repoRoot, command.releaseDir)}`);
}
async function buildRelease() {
    ...
    await fs.writeFile(path.join(releaseConfigDir, 'bootstrap.env'), serializeBootstrapEnv(bootstrapEnvConfig), 'utf8');
    console.log('[agent:release] generated release/config/bootstrap.env from effective environment');
    console.log(`[agent:release] deploying production dependencies from workspace "${agentDeployWorkspace}"`);
